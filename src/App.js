...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (min-width: 768px) {
    ...
    .map-container { margin: 0 32px 16px; }
    .map-filters { padding: 0 32px 16px; }
    ...
    .quick-actions .btn-primary, .quick-actions .btn-secondary { max-width: 200px; }
  ...
  .sidebar {
    ...
    background: var(--white); border-right: 1px solid var(--light);
    padding: 32px 16px; gap: 4px;
    box-shadow: 2px 0 12px rgba(0,0,0,0.04);
  }
  .sidebar-logo {
    font-family: 'Bebas Neue', sans-serif; font-size: 32px;
    color: var(--dark); padding: 0 12px 24px; letter-spacing: 0.02em;
  }
  ...
  /* ── HERO ── */
  .hero {
    background: linear-gradient(150deg, #3B9EE8 0%, #2B8ED8 100%);
  ...
  .tide-times { display:flex; flex-wrap:wrap; padding:4px 0; }
  ...
  .catch-card-img { width:100%; height:100px; object-fit:cover; display:block; }
  ...
  /* ── MAP ── */
  .map-container { margin:0 16px 0; border-radius:20px; overflow:hidden; border:1px solid var(--light); height:420px; box-shadow:var(--shadow); position:relative; }
  .map-container iframe { width:100%; height:100%; border:none; display:block; }
  .map-view-toggle { display:flex; gap:8px; padding:12px 16px 8px; }
  .map-toggle-btn { font-size:13px; font-weight:600; padding:8px 18px; border-radius:20px; border:2px solid var(--light); background:var(--white); color:var(--mid); cursor:pointer; transition:all 0.2s; }
  .map-toggle-btn.active { border-color:var(--blue); color:var(--blue); background:rgba(59,158,232,0.05); }
  .map-filters { display:flex; gap:8px; padding:0 16px 16px; flex-wrap:wrap; }
  .map-pill { font-size:13px; font-weight:600; padding:8px 16px; border-radius:20px; border:2px solid var(--light); background:var(--white); color:var(--mid); cursor:pointer; transition:all 0.2s; }
  .map-pill.active { border-color:var(--blue); color:var(--blue); background:rgba(59,158,232,0.05); }
  ...
  .map-picker { margin-top:12px; border-radius:14px; overflow:hidden; border:2px solid var(--blue); height:240px; position:relative; }
  ...
  .pin-confirm { margin-top:8px; padding:10px 14px; background:rgba(59,158,232,0.08); border-radius:10px; font-size:13px; color:var(--blue); font-weight:600; text-align:center; }
  ...
  .item-detail-img-wrap { width:100%; height:200px; border-radius:18px; background:#E8F4FD; display:flex; align-items:center; justify-content:center; margin-bottom:20px; border:2px solid var(--light); overflow:hidden; }
  ...
  .item-detail-price { font-size:20px; font-weight:700; color:var(--blue); margin-top:10px; }
  ...
  .catch-history-emoji { font-size:24px; }
  ...
  .stat-card:hover { border-color:var(--blue); transform:translateY(-2px); box-shadow:var(--shadow2); }
  ...
  .catch-drill-card { background:var(--white); border:2px solid var(--light); border-radius:16px; overflow:hidden; box-shadow:var(--shadow); cursor:pointer; transition:all 0.2s; }
  .catch-drill-card:hover { border-color:var(--blue); transform:translateY(-2px); }
  .catch-drill-img { width:100%; height:90px; object-fit:cover; background:#E8F4FD; display:flex; align-items:center; justify-content:center; }
  ...
  .session-row { display:flex; align-items:center; gap:14px; padding:14px 0; border-bottom:1px solid var(--light); cursor:pointer; }
...
// ── FISH PHOTOS (Wikimedia Commons public domain) ─────────────────────────
const FISH_PHOTOS = {
  'Sea Bass': 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3b/Dicentrarchus_labrax.jpg/320px-Dicentrarchus_labrax.jpg',
  'Cod': 'https://upload.wikimedia.org/wikipedia/commons/thumb/2/twentyf/Atlantic_cod.jpg/320px-Atlantic_cod.jpg',
  ...
  'default': 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3b/Dicentrarchus_labrax.jpg/320px-Dicentrarchus_labrax.jpg',
};

function FishImg({ species, className, style }) {
  const [err, setErr] = useState(false);
  const src = FISH_PHOTOS[species] || FISH_PHOTOS['default'];
...
const SESSIONS = [
  { id:1, location:'Chesil Beach', date:'10 Jun 2025', catches:3, emoji:'🌊' },
  { id:2, location:'Whitby Pier', date:'9 Jun 2025', catches:2, emoji:'🏗️' },
  { id:3, location:'Brighton Marina', date:'5 Jun 2025', catches:5, emoji:'⚓' },
  { id:4, location:'River Itchen', date:'28 May 2025', catches:4, emoji:'🏞️' },
];
...
const INITIAL_TACKLE = [
  { id:'rods', name:'Rods', emoji:'🎣', items:[
    { id:'r1', name:'Daiwa Ninja X', brand:'Daiwa', price:'£89.99', catches:[{ species:'Sea Bass', weight:'4lb 8oz', date:'10 Jun 2025' },{ species:'Pollock', weight:'2lb 4oz', date:'3 Mar 2025' }] },
    { id:'r2', name:'Shakespeare Ugly Stik', brand:'Shakespeare', price:'£54.99', catches:[{ species:'Cod', weight:'5lb 0oz', date:'8 Jan 2025' }] },
  ]},
  { id:'reels', name:'Reels', emoji:'⚙️', items:[{ id:'re1', name:'Shimano Baitrunner DL', brand:'Shimano', price:'£129.99', catches:[] }] },
...
const PBS = [
  { species:'Sea Bass', weight:'7lb 2oz' },
  { species:'Cod', weight:'9lb 4oz' },
  { species:'Mackerel', weight:'1lb 14oz' },
  { species:'Pollock', weight:'4lb 8oz' },
  { species:'Smoothhound', weight:'11lb 6oz' },
  { species:'Brown Trout', weight:'3lb 2oz' },
  { species:'Pike', weight:'14lb 8oz' },
  { species:'Barbel', weight:'8lb 12oz' },
];
...
function TideSection() {
  ...
  useEffect(() => {
    function generateTides(locName) {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        pos => {
          ...
          const loc = latitude > 53 ? 'Northern England' : latitude > 51.5 ? 'Southern England' : 'South West England';
          generateTides(loc);
        },
        () => generateTides('Southampton')
      );
    } else generateTides('Southampton');
  }, []);
        ...
        {RECENT_CATCHES.map(c => (
          <div className="catch-card" key={c.id}>
            <div className="catch-card-img-wrap">
              <FishImg species={c.species} className="catch-card-img" />
            </div>
...
function MapScreen() {
  ...
  const [userLat, setUserLat] = useState(51.5);
  const [userLon, setUserLon] = useState(-1.0);
  ...
  const bbox = `${userLon-0.3},${userLat-0.2},${userLon+0.3},${userLat+0.2}`;
  const layer = mapView === 'Satellite' ? 'hot' : 'mapnik';
  const mapSrc = `https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&layer=${layer}&marker=${userLat},${userLon}`;
  ...
  return (
    <div>
      ...
      <div className="map-view-toggle">
        ...
        {located && <span style={{fontSize:12,color:'var(--blue)',fontWeight:600,marginLeft:4,alignSelf:'center'}}>📍 Your location</span>}
      </div>
      <div className="map-container">
        <iframe title="Map" sandbox="allow-scripts allow-same-origin" src={mapSrc} loading="lazy"/>
      </div>
...
function LogScreen({ onSaved }) {
  ...
  const [userLon, setUserLon] = useState(-1.0);
  useEffect(() => {
    if (navigator.geolocation) navigator.geolocation.getCurrentPosition(p => { setUserLat(p.coords.latitude); setUserLon(p.coords.longitude); });
  }, []);

  const bbox = `${userLon-0.5},${userLat-0.35},${userLon+0.5},${userLat+0.35}`;
  const mapSrc = `https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&layer=mapnik&marker=${userLat},${userLon}`;
      ...
      <div className="log-form">
        ...
        <div className="form-group">
          ...
          <div className="map-picker" style={{marginTop:12}}>
            <iframe title="Pin location" sandbox="allow-scripts allow-same-origin" src={mapSrc} loading="lazy"/>
          </div>
          <div className="map-picker-hint">
            {form.privacy==='Approximate' ? '🔒 Your exact location will be blurred to a ~2 mile zone' : '📍 Tap the map to drop your pin at the exact catch location'}
          </div>
        </div>
...
function TackleScreen() {
  ...
  const saveNewItem = () => {
    ...
    const item = { id:Date.now().toString(), name:newItem.name, brand:newItem.brand, price:newItem.price?`£${newItem.price}`:'Price not set', catches:[] };
      ...
      <div>
        ...
        <div className="item-detail">
          <div className="item-detail-img-wrap">
            <div className="item-detail-img-emoji">{cat.emoji}</div>
          </div>
          ...
          <div className="detail-section">
            <div className="detail-section-title">Catches with this {cat.name.slice(0,-1)}</div>
      ...
      <div>
        ...
        <div className="item-grid">
          ...
          <div className="add-item-card" onClick={() => setShowAddItem(true)}>
            <span>➕</span><p>Add {cat.name.endsWith('s') ? cat.name.slice(0,-1) : cat.name}</p>
          </div>
        </div>
        {showAddItem && (
          <div style={{padding:'20px 16px 0'}}>
            <div style={{background:'var(--white)',borderRadius:18,padding:20,boxShadow:'var(--shadow)',border:'1px solid var(--light)'}}>
              <div style={{fontSize:16,fontWeight:700,marginBottom:16,color:'var(--dark)'}}>Add New Item</div>
              <div className="form-group"><label className="form-label">Name</label><input className="form-input" placeholder="e.g. Daiwa Ninja X" value={newItem.name} onChange={e => setNewItem(n=>({...n,name:e.target.value}))} /></div>
              <div className="form-group"><label className="form-label">Brand</label><input className="form-input" placeholder="e.g. Daiwa" value={newItem.brand} onChange={e => setNewItem(n=>({...n,brand:e.target.value}))} /></div>
              <div className="form-group"><label className="form-label">Price paid (£)</label><input className="form-input" placeholder="e.g. 89.99" value={newItem.price} onChange={e => setNewItem(n=>({...n,price:e.target.value}))} /></div>
  ...
  return (
    <div>
      <div className="section-header" style={{paddingTop:24}}>
        ...
        <button onClick={() => setShowAddCat(true)} style={{background:'none',border:'none',fontSize:22,color:'var(--blue)',cursor:'pointer',padding:'0 4px',lineHeight:1}}>+</button>
      </div>
      <div className="tackle-grid">
        {categories.map(cat => (
          <div className="tackle-cat" key={cat.id} onClick={() => setSelectedCat(cat.id)}>
            ...
            <div className="tackle-bar"><div className="tackle-bar-fill" style={{width:`${Math.min(100,(cat.items.length/8)*100)}%`}}/></div>
          </div>
        ))}
      </div>
      {showAddCat && (
        <div style={{padding:'20px 16px 0'}}>
          <div style={{background:'var(--white)',borderRadius:18,padding:20,boxShadow:'var(--shadow)',border:'1px solid var(--light)'}}>
            <div style={{fontSize:16,fontWeight:700,marginBottom:16,color:'var(--dark)'}}>Add New Category</div>
...
function StatsScreen() {
  ...
  if (view==='catches') {
    return (
      <div>
        ...
        <div className="section-header" style={{paddingTop:4}}>
          <span className="section-title">All Catches</span>
        </div>
        <div className="catches-grid">
          {ALL_CATCHES.map(c => (
            <div className="catch-drill-card" key={c.id}>
              <div className="catch-drill-img">
                <FishImg species={c.species} style={{width:'100%',height:'100%',objectFit:'cover'}} />
              </div>
              <div className="catch-drill-body">
                <div className="catch-drill-species">{c.species}</div>
                <div className="catch-drill-weight">{c.weight}</div>
                <div className="catch-drill-meta">📍 {c.location}<br/>🕐 {c.date}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  if (view==='species') {
    const species = [...new Set(ALL_CATCHES.map(c => c.species))];
    return (
      <div>
        <button className="back-btn" onClick={() => setView('main')}>← Back to Stats</button>
        <div className="section-header" style={{paddingTop:4}}>
          <span className="section-title">Species Caught</span>
        </div>
        <div className="catches-grid">
          {species.map(s => {
            const count = ALL_CATCHES.filter(c => c.species===s).length;
            return (
              <div className="catch-drill-card" key={s}>
                <div className="catch-drill-img">
                  <FishImg species={s} style={{width:'100%',height:'100%',objectFit:'cover'}} />
                </div>
                <div className="catch-drill-body">
                  <div className="catch-drill-species">{s}</div>
                  <div className="catch-drill-weight">{count} catch{count!==1?'es':''}</div>
                </div>
              </div>
            );
          })}
        ...
        <div className="sessions-list">
          {SESSIONS.map(s => (
            <div className="session-row" key={s.id}>
              <div className="session-icon">{s.emoji}</div>
              <div>
                <div className="session-name">{s.location}</div>
                <div className="session-meta">{s.date}</div>
              </div>
              <div className="session-count">{s.catches} catches</div>
            </div>
          ))}
        </div>
        <div style={{height:16}}/>
  ...
  return (
    <div>
      ...
      <div className="stats-grid">
        <div className="stat-card" onClick={() => setView('catches')}>
          <div className="stat-num">47</div>
          <div className="stat-label">Total Catches ›</div>
        </div>
        <div className="stat-card" onClick={() => setView('species')}>
          <div className="stat-num">11</div>
          <div className="stat-label">Species Caught ›</div>
        </div>
        <div className="stat-card">
          <div className="stat-num">3.8</div>
          <div className="stat-label">Avg Weight (lb)</div>
        </div>
        <div className="stat-card" onClick={() => setView('sessions')}>
        ...
        {PBS.map((pb,i) => (
          <div className="pb-row" key={i}>
            <div className="pb-left">
              <FishImg species={pb.species} className="pb-fish-img" />
...
export default function App() {
  ...
  const renderScreen = () => {
    switch(screen) {
      ...
      case 'log':    return <LogScreen onSaved={() => { triggerToast('🐟 Catch saved!'); setScreen('home'); }} />;
    ...
    <>
      ...
      <div className="app">
        ...
        <nav className="sidebar">
          ...
          {NAV.map(n => (
            <button key={n.id} className={`sidebar-item${screen===n.id?' active':''}`} onClick={() => setScreen(n.id)}>
              {n.icon}{n.label}
            </button>
          ))}
